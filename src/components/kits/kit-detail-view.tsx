  ...
  if (kit.data.status !== 'completed' || !kit.data.kit) {
    return (
      <PageContainer className="max-w-4xl">
        <PageHeader title="Preparing your interview kit" description="Research and generation can take a little while. Progress below reflects backend stages." />
        {status.data ? <GenerationProgress status={status.data} /> : <LoadingState rows={4} />}
        {status.isError ? <div className="mt-6"><ErrorState message={getErrorMessage(status.error)} onRetry={() => status.refetch()} /></div> : null}
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Company brief</CardTitle></CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <Badge variant={data.coverage.uncovered_requirement_ids.length === 0 ? 'default' : 'secondary'}>
          ...
          {data.role.requirements.map((requirement) => (
            <Card key={requirement.id} className="gap-3 py-4">
              <CardContent className="flex items-start justify-between gap-3">
                ...
                <Badge variant={requirement.priority === 'must' ? 'default' : 'outline'}>{requirement.priority}</Badge>
              </CardContent>
            </Card>
          ))}
        </div>
          ...
          {data.schedule.days.map((day) => (
            <Card key={day.day} className="gap-3 py-4">
              <CardContent>
                ...
                <p className="mt-2 text-sm text-muted-foreground">{day.minutes} minutes · {day.question_ids.length} questions</p>
              </CardContent>
            </Card>
          ))}
        </div>
